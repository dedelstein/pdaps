...
#BSUB -B
#BSUB -N
#BSUB -o logs/pdaps_v7_accel4.%J.out
#BSUB -e logs/pdaps_v7_accel4.%J.err

set -euo pipefail

ACCEL=4
ROOT_ID=${PDAPS_ROOT_ID:-${LSB_JOBID:-$(date +%Y%m%d_%H%M%S)}}
OUT_ROOT=${PDAPS_OUT_ROOT:-results/mri_validation_pdaps_v7_accel${ACCEL}_${ROOT_ID}}
mkdir -p "$OUT_ROOT"

METHOD_SELECTION=${METHOD_INDICES:-0-19}
...
echo "PDAPS v7 validation-only ablation: accel=${ACCEL}, methods=${METHOD_SELECTION}, out=${OUT_ROOT}"

./.venv/bin/python3 mri_validation.py \
    --grid-preset pdaps_v7 \
