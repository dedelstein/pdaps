...
#BSUB -B
#BSUB -N
#BSUB -o logs/pdaps_v4_accel8.%J.out
#BSUB -e logs/pdaps_v4_accel8.%J.err

set -euo pipefail

STAMP=${STAMP:-${LSB_JOBID:-$(date +%Y%m%d_%H%M%S)}}
OUT_ROOT=${PDAPS_OUT_ROOT:-results/mri_validation_pdaps_v4_accel8_${STAMP}}
mkdir -p "$OUT_ROOT"

echo "PDAPS v4 ablation: accel=8, out=${OUT_ROOT}"
