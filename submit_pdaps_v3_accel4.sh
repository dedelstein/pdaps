...
#BSUB -B
#BSUB -N
#BSUB -o logs/pdaps_v3_accel4.%J.out
#BSUB -e logs/pdaps_v3_accel4.%J.err

set -euo pipefail

STAMP=${STAMP:-${LSB_JOBID:-$(date +%Y%m%d_%H%M%S)}}
OUT_ROOT=${PDAPS_OUT_ROOT:-results/mri_validation_pdaps_v3_accel4_${STAMP}}
mkdir -p "$OUT_ROOT"

echo "PDAPS v3 ablation: accel=4, out=${OUT_ROOT}"
