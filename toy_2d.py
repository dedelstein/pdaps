...
Clean two-scenario toy suite for comparing DPS, DAPS, pULA, and P-DAPS.

Toy A is pushed toward mode-selection difficulty.
Toy B is pushed toward stiffness / preconditioning difficulty.
"""
...
class DivergenceError(RuntimeError):
    ...
    Carries the step size and the max absolute value observed so stability
    sweeps can record the exact boundary where integration fails.
    """
...
def _diverged_result(x_last, progress, exc, step_idx):
    """Build a result dict that marks the run as diverged without crashing the sweep."""
...
def _langevin_inner_preconditioned(x0_hat, y, sigma, num_steps=20, step_size=0.5):
    var = sigma ** 2
    A_s = A / SIGMA_NOISE
    y_s = y / SIGMA_NOISE
    AtA = A_s.T @ A_s
    ...
    for _ in range(num_steps):
        residual = y_s - x @ A_s.T
        lik_score = residual @ A_s
        prior_score = -(x - x0_hat.detach()) / var
        ...
        n1 = torch.randn(A_s.shape[0], nb, dtype=DTYPE)
        n2 = torch.randn(DIM, nb, dtype=DTYPE)
        z = (M @ (A_s.T @ n1 + n2 / sigma)).T
        x = x.detach() + (step_size / 2) * conv_grad + np.sqrt(step_size) * z

    _check_divergence(x, step_size, "pdaps_langevin")
    return x.detach()


def _langevin_inner_preconditioned_warm(x_init, x0_hat, y, sigma, num_steps=20, step_size=0.5):
    var = sigma ** 2
    A_s = A / SIGMA_NOISE
    y_s = y / SIGMA_NOISE
    AtA = A_s.T @ A_s
    ...
    _check_divergence(x, step_size, "pdaps_langevin_warm")
    return x.detach()
...
def _operator_space_metrics(samples_np):
    ...
    # sampler-mean residual
    mean_x = x.mean(dim=0)
    resid_mean = y - A @ mean_x
    chi2_mean = float((resid_mean ** 2).sum().item() / sig2)
    # log-evidence (numerically stable via logsumexp)
    pred_y = x @ A.T  # (n, m)
...
def _summarize_method_result(method, result, gt_summary):
    ...
    # bures2_x2: 2-Wasserstein between Gaussian approximations of (mean, std_x2)
    mr = fit["mean_rmse"]
    sx = fit["std_x2_error"]
    bures2 = float(np.sqrt(mr ** 2 + sx ** 2)) if np.isfinite(mr) and np.isfinite(sx) else np.nan
    # When a run diverged, the sample-derived metrics describe the fallback x0_hat
    # (the clean ODE estimate at the divergence step), not the sampler's stationary
    # distribution. Mixing those into aggregate means produces a misleading floor in
    # stability sweeps. Mark them NaN so np.nanmean drops them; the `diverged` flag
    # and `diverged_step_size` columns carry the breakdown signal explicitly.
    return {
        "method": method,
